package br.com.alg.giraofoodapi.util;
...
public class DatabaseCleaner {
    ...
    private void addTruncateSatements(List<String> tableNames, Statement statement) {
        tableNames.forEach(tableName -> {
            try {
                ...
                if(!this.getListTablesIgnored().contains(tableName)) {
                    statement.addBatch(sql("ALTER SEQUENCE " + tableName + "_seq RESTART WITH 1 "));
                }
            } catch (SQLException e) {
                throw new RuntimeException(e);
            }
    ...
    private List<String> getListTablesIgnored() {
        return Arrays.asList("grupo_permissao", "restaurante_forma_pagamento", "usuario_grupo");
    }
